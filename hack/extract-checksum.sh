...
# Original file from Mike Farah https://github.com/mikefarah/yq/blob/master/scripts/extract-checksum.sh

# This script works with checksums_hashes_order and checksums to extract the relevant
# sha of the various krmgen downloads. You can then use your favourite checksum tool to validate.
# <CHECKSUM> must match an entry in checksums_hashes_order.
#
# Usage: ./extract-checksum.sh <CHECKSUM> <FILE>
# E.g: ./extract-checksum.sh SHA-256 krmgen_linux_amd64.tar.gz
# Outputs:
# krmgen_linux_amd64.tar.gz	acebc9d07aa2d0e482969b2c080ee306e8f58efbd6f2d857eefbce6469da1473
#
# Usage with rhash:
# ./extract-checksum.sh SHA-256 krmgen_linux_amd64.tar.gz | rhash -c -
#
...
#
# ./extract-checksum.sh MD5 krmgen_linux_amd64.tar.gz | awk '{ print $2 " " $1}' | md5sum -c -
#
#
...
if [ ! -f "checksums_hashes_order" ]; then
  ...
  echo "Download the file from https://github.com/librucha/krmgen/releases/ for the version of krmgen you are trying to validate"
  exit 1
fi
